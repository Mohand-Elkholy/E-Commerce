...
import styles from "./WomenSlider.module.css"
...
export default function WomenSlider() {
    ...
    const filteredData = data?.data.data.filter((proudct)=>proudct.category.name == "Women's Fashion")
return (
    <>
    {!isLoading ? <>
    <h2 className=' font-semibold text-lg mt-10 mb-5 dark:text-slate-100'>Shop Popular Women's Fashion :</h2>
    <Slider {...settings}>
          ...
          <div className='p-2 border rounded-lg'>
              <div className=' relative'>
                ...
                <div className='flex items-center justify-between dark:text-slate-100'>
                    <p className='mt-2 text-xs md:text-base lg:text-lg'><span>EGP</span> <span>{proudct.priceAfterDiscount} </span> <span className={proudct.priceAfterDiscount?'line-through text-gray-300 dark:text-slate-400':''}>{proudct.price}</span> {proudct.priceAfterDiscount?<span className=' text-[#9c34c2]'>{100-Math.ceil(proudct.priceAfterDiscount/proudct.price*100)}%</span> : ""}</p>
                </div>
